...
fn render_help<B: Backend>(f: &mut Frame<B>, area: Rect, state_stack_size: usize) {
    ...
    help.push(Spans(vec![Span::raw(
        "0..9: set panel count (where applicable)\n",
    )]));
    help.push(Spans(vec![Span::raw("X: Export\n")]));

    help.push(Spans(vec![]));
    help.push(Spans(vec![Span::raw(format!(
        "Saved states: {state_stack_size}"
    ))]));
    let para = Paragraph::new(help).wrap(Wrap { trim: false });
    f.render_widget(para, area);
}

fn render_errors<'a, B: Backend, H: Hyphenator<'a, Opportunity = usize>>(
    f: &mut Frame<B>,
    area: Rect,
    errors: &[PanelError],
    hyphenator: &'a H,
...
impl tui::widgets::Widget for ColorPicker {
    fn render(self, area: Rect, buf: &mut tui::buffer::Buffer) {
        ...
        for color in COLORS {
            ...
            if color == self.color {
                buf.set_string(
                    color_box.left() + color_width / 2,
                    color_box.top(),
                    "*",
                    Style::default().fg(color.complement().to_tui()),
                );
            }
...
pub fn ui<'a, G: Gridlike<'a>, B: Backend>(
    ...
    f.render_stateful_widget(
        GridRenderer::new(&*grid),
        block.inner(chunks[0]),
        &mut (cx, cy, tagged, error_locs),
    );
